...
BytecodeSeq *compileBytecodeSeq;

static BytecodeSeq *currentBytecodeSeq() {
  return &current->function->bytecodeSeq;
}
...
#ifdef DEBUG_PRINT_CODE
  if (!parser.hadError) {
    disassembleBytecodeSeq(currentBytecodeSeq(), function->name != NULL
                                                     ? function->name->chars
                                                     : "<script>");
  }
#endif
